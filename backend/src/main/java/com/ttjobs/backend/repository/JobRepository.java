...
public interface JobRepository extends JpaRepository<Job, Long>, JpaSpecificationExecutor<Job> {
    ...
    @Query("SELECT j as job, COUNT(sj.id) as savedCount " +
           ...
           "GROUP BY j " +
           "ORDER BY COALESCE(j.salaryMax, j.salary, j.salaryMin, 0) DESC, COUNT(sj.id) DESC, j.postedDate DESC, j.id DESC")
    ...
    @Query("SELECT j as job, COUNT(sj.id) as savedCount " +
           ...
           "WHERE j.deletedAt IS NULL AND j.company.deletedAt IS NULL AND LOWER(j.status) = LOWER(:status) " +
           "GROUP BY j " +
           "ORDER BY COUNT(sj.id) DESC, j.postedDate DESC, j.id DESC")
